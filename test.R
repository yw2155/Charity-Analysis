...
query = "SELECT a.contact_id,
                DATEDIFF(20121101, MAX(a.act_date)) / 365 AS 'recency',
         ...
         LEFT JOIN (SELECT contact_id, COUNT(amount) AS counter, AVG(amount) AS targetamount
                    FROM acts
                    WHERE (act_date >= 20121101) AND
                          (act_date <  20131101) AND
                          (act_type_id = 'DO')
                    GROUP BY contact_id) AS c
         ON c.contact_id = a.contact_id
         WHERE (act_type_id = 'DO') AND (act_date < 20121101)
         GROUP BY 1"
...
query = "SELECT contact_id,
                DATEDIFF(20131101, MAX(act_date))
                   / 365 AS 'recency',
